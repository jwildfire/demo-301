...
- **`Build Site` can run, and does.** It had zero runs ever, and could not have succeeded from anywhere: the `github-pages` environment admits only `gh-pages` and `site`, and Pages here publishes the `site` branch directly rather than through Actions. Its five-file contract check is now a job that runs on every push to `site` and refuses a half-written tree; the deploy half is kept, dormant, in case Pages is ever moved onto it.
